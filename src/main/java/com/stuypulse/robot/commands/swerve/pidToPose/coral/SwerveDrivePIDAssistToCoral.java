...
import com.stuypulse.robot.subsystems.vision.GamepieceVision;
...
public class SwerveDrivePIDAssistToCoral extends Command {
    ...
    private final Pose2d coralPose;
    private final FieldObject2d targetPose2d;

    public SwerveDrivePIDAssistToCoral(Gamepad driver) {
        swerve = CommandSwerveDrivetrain.getInstance();
        this.driver = driver;
        coralPose = GamepieceVision.getInstance().getCoralPose();
    ...
    @Override
    public void execute() {
        targetPose2d.setPose(Robot.isBlue() ? coralPose : Field.transformToOppositeAlliance(coralPose));

        controller.update(coralPose, swerve.getPose());
        ...
        SmartDashboard.putNumber("Alignment/Target x", coralPose.getX());
        SmartDashboard.putNumber("Alignment/Target y", coralPose.getY());
        SmartDashboard.putNumber("Alignment/Target angle", coralPose.getRotation().getDegrees());
